...
test("data modified in event listener updates effected attribute bindings", async () => {
    ...
    await wait(() => {
        expect(document.querySelector("span").getAttribute("foo")).toEqual(
            "baz"
        );
    });
});
...
test("click away", async () => {
    ...
    expect(document.querySelector("ul").classList.contains("hidden")).toEqual(
        false
    );
    ...
    await wait(() => {
        expect(
            document.querySelector("ul").classList.contains("hidden")
        ).toEqual(false);
    });
